<template>
  <v-card flat>
    ...
    <v-data-table
      ...
      multi-sort
      :search="search"
      class="elevation-0 nsys-data-table"
      hide-default-footer
    >
      <template v-slot:item.name="{ item }">
        <router-link
          class="link"
          :to="{ name: 'administration-organisation-details', params: { uuid: item.uuid }}"
        >{{ item.name }}</router-link>
      </template>
      <template v-slot:item.validated="{ item }">
        <v-icon small v-show="item.validated" color="success">mdi-check</v-icon>
      </template>
    </v-data-table>
  </v-card>
</template>
